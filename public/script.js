    ...
    sendResult.innerHTML = `
      <div class="flex flex-col items-center">
        ...
        <button onclick="copyToClipboard('${data.code}')" 
          class="bg-gray-100 px-3 py-1 rounded text-sm hover:bg-gray-200 dark:hover:bg-gray-600">
          📋 Copy Code
        </button>
      </div>
    `;
  ...
  try {
    ...
    if (data.files && data.files.length > 0) {
      fileLinks.innerHTML = data.files.map(file =>
        `<a href="${file.url}" download class="flex items-center gap-2 text-blue-600 hover:text-blue-800 dark:hover:text-blue-300 transition-colors p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-600">
